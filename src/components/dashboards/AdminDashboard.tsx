...
import { Users, Key, Wallet, FileText } from 'lucide-react';

export default function AdminDashboard() {
  const [stats, setStats] = useState({ users: 0, keys: 0, activeKeys: 0, totalBalance: 0 });

  useEffect(() => {
    const fetchStats = async () => {
      const [usersRes, keysRes, activeKeysRes, balanceRes] = await Promise.all([
  ...
  const cards = [
    ...
    { title: 'Total Wallet Balance', value: `₹${stats.totalBalance.toLocaleString('en-IN')}`, icon: <Wallet className="h-5 w-5" />, color: 'text-warning' },
  ];
