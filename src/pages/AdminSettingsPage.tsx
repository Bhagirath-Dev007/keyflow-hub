...
export default function AdminSettingsPage() {
  ...
  const fetchQr = async () => {
    const { data } = await supabase.from('admin_settings').select('setting_value').eq('setting_key', 'payment_qr_url').single();
      ...
      <div className="animate-fade-in">
        ...
        <Card className="max-w-lg">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">Upload a QR code image that resellers will see when adding wallet balance.</p>
            {qrUrl ? (
              <div className="space-y-3">
                <img src={qrUrl} alt="Payment QR" className="mx-auto max-w-[250px] rounded-xl border" />
                <div className="flex gap-2 justify-center">
                  <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()}><Upload className="mr-2 h-4 w-4" />Replace</Button>
                  <Button variant="destructive" size="sm" onClick={handleRemoveQr}><Trash2 className="mr-2 h-4 w-4" />Remove</Button>
                </div>
              </div>
            ) : (
              <div onClick={() => fileRef.current?.click()} className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-muted-foreground/30 p-10 transition-colors hover:border-primary hover:text-primary">
                <Upload className="h-8 w-8 text-muted-foreground" />
                <p className="text-sm text-muted-foreground">Click to upload QR code</p>
              </div>
            )}
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) handleUploadQr(file); e.target.value = ''; }} />
